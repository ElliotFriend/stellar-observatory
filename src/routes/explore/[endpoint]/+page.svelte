...
<div class="mx-auto max-w-4xl px-4 py-12">
	<a href="/" class="mb-6 inline-flex items-center text-sm text-slate-400 hover:text-nebula-400 transition-colors">
		...
		<div class="flex items-center gap-4 mb-3">
			...
			<div>
				<h2 class="text-3xl font-bold text-white">
					{endpoint.slug.split('-').map((w: string) => w[0].toUpperCase() + w.slice(1)).join(' ')}
				</h2>
	...
	<div class="mb-6 flex items-center gap-4">
		<button
			...
			class="rounded-lg bg-nebula-600 px-6 py-2.5 text-sm font-medium text-white transition-all
				hover:bg-nebula-500 disabled:opacity-50 disabled:cursor-not-allowed"
		>
			{#if loading}
