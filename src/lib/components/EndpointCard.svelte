<script lang="ts">
	...
	let { endpoint, preview }: { endpoint: EndpointConfig; preview?: Record<string, unknown> } = $props();
</script>
...
>
	...
	<h3 class="mb-2 text-lg font-semibold text-white group-hover:text-nebula-400 transition-colors">
		{endpoint.slug.split('-').map(w => w[0].toUpperCase() + w.slice(1)).join(' ')}
	</h3>
			...
			<div class="space-y-1 text-slate-400">
				{#each Object.entries(preview) as [key, value]}
					<div class="flex justify-between">
						...
						<span class="text-slate-300">{typeof value === 'number' ? value.toLocaleString() : value}</span>
					</div>
				{/each}
			</div>
		</div>
